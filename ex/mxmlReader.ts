import { TileLevel } from "./tilelevel5";
...
export function all(name, tree): TreeValue[] {
	let r: TreeValue[] = [];
	for (let i = 0; i < tree.children.length; i++) {
		if (tree.children[i].name == name) {
...
export function onFileLoad(action: (treeValue: TreeValue) => {}) {
...
export function readDocChildren(node): TreeValue[] {
...
var levelEngine = new TileLevel((window.document.getElementById('contentSVG') as any) as SVGElement);
var smallSize = 0.1;
		var cellSize = 0.6;
